peweproxy.register_module('timelog', function($) {
	var period = 8
	
	var active_in_last_period = false;
	var num_of_scrolls = 0;
	var num_of_copies = 0;
	...
	var upload_activity = function() {
		if (active_in_last_period) {
			$.post('http://' + base + '/activity/update?nologging', { 'period': period, 
				'scrolls': num_of_scrolls, 'copies': num_of_copies, 'nologging': 'true', 'access_guid' : peweproxy.access_id });
